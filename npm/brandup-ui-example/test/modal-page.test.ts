...
// Страницу убираем сразу после теста, а не только перед следующим: за последним теста нет, и она
// доживает в DOM до сноса jsdom. Разбирая документ, jsdom удаляет её узлы, наблюдатель @brandup/ui
// ставит на это авторазрушение контролов — и оно выполняется, когда `document` в песочнице уже
// отобран. Снятие слушателя падает, а сообщить об ошибке jsdom не может: окно закрыто, и его
// собственный обработчик спотыкается о `window._document`. Наружу это выходит не упавшим тестом,
// а убитым процессом: «Jest worker encountered 4 child process exceptions».
afterEach(() => {
	document.body.innerHTML = "";
});
