...
| `onChange(handler)` | Подписка на событие `ui:richeditor:change` |
...
`getValue()` считает значение по DOM и точен всегда. А вот **уведомление** `ui:richeditor:change` при печати доставляется с задержкой: сериализация — самая дорогая операция редактора (обход всего содержимого), а печать даёт `input` на каждый символ.
...
Ссылки без текста или без адреса не бывает — такая разметка остаётся текстом. Перенос строки внутри ссылки тоже: её текст в разметке один кусок, разорванный он не выражается и с разбора не вернётся. **Enter** внутри ссылки её заканчивает — за переносом остаётся обычный текст, а не вторая ссылка с тем же адресом. Схема адреса проверяется у самой записи в разметку (`safeUrl`), поэтому одинаково на всех путях: разбор значения, вставка из буфера и правка адреса в панели. `javascript:`, `data:` и прочее исполняемое ссылкой не становится — такой адрес равносилен пустому, то есть ссылки с ним не будет ни в поле, ни в значении. Разрешены `http`, `https`, `mailto`, `tel` и адреса без схемы вовсе — относительные и протокол-относительные.
...
Ссылка — оформление текста, а не вставка: делать ссылкой нечего — кнопка погашена, и `applyLink` ничего не делает. Негодный адрес `applyLink` понимает как пустой: новой ссылки не будет, а у готовой, которой правили адрес, ссылка снимется — поле показывает ровно то, что уйдёт хосту.
...
import ru from "@brandup/ui-richeditor/locale/ru.json";

setTexts(ru);
```

Подпись отдельного контрола задаётся на нём самом и сильнее текстов приложения. Панель форматирования и панель смайликов берут свои подписи отсюда же. Подробнее —
