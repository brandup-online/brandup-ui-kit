...
Редактор текста на базе `contenteditable`. Получает в конструкторе элемент, делает его редактируемым и берёт на себя всю работу с вводом: форматирование (жирный, курсив, зачёркивание, подчёркивание), плавающую панель инструментов, режим набора, нормализацию пробелов и сериализацию значения в HTML или Markdown.
...
Конструктор оборачивает переданный элемент в `div.ui-richeditor` (к нему привязан `UIElement`), делает элемент редактируемым (`div.ui-richeditor-input`) и добавляет плавающую панель форматирования, которая появляется в фокусе.
...
Подключается `richeditor.less`. Цветовые переменные `--input-*` берутся из `@brandup/ui-kit` (с fallback-значениями для standalone). Состояния: класс `focused` на обёртке (показывает панель), `active` на кнопке инструмента (формат активен).
