...
| `executing` | То же ожидание, но класс ставит `@brandup/ui`, а не хост — см. ниже |
...
Кнопке с командой ставить класс руками не нужно. Пока асинхронная команда не завершилась —
обработчик вернул `Promise`, — `@brandup/ui` держит на её элементе класс `executing`, и кит рисует
его так же, как `loading`:

```typescript
this.registerCommand("save", async () => {
    await api.save(); // на это время кнопка с data-command="save" показывает кольцо
});
```

`aria-busy` библиотека при этом не ставит — если он нужен, его выставляет сама команда.
...
| `--hover--button-tint-color` / `--active--button-tint-color` | `#000` / `#000` — что подмешивается: чёрный темнит, белый светлит |
...
Под курсором и при нажатии кнопка темнеет: к заливке подмешивается `--hover--button-tint-color`
на долю `--hover--button-tint`. Тёмной заливке темнеть некуда — отрицательной доли не бывает,
`color-mix` такого не принимает, — поэтому светление задаётся цветом. Токен обычный, так что
переопределяется и на отдельном виде:

```css
.ui-button.primary {
    --hover--button-tint-color: #fff;
    --active--button-tint-color: #fff;
    --hover--button-tint: 8%; /* белый заметнее чёрного — долю берут меньше */
}
```

Тем же цветом отвечают на курсор отмеченный чекбокс (`--hover--checkbox-tint-color`) и кнопка
тулбара поля ввода (`--hover--input-toolbar-button-tint-color`): в Less они выведены из кнопочного,
поэтому тема, собранная из исходников, задаёт одно значение на всех. На живой странице каждый
переопределяется отдельно — но кнопочный и чекбоксовый действуют где угодно, вплоть до одного
элемента, а тулбарный только в `:root`: его заливка там же и выводится, и в поддереве считаться
заново не будет. Поддереву нужен готовый `--hover--input-toolbar-button-fill`.
...
элемент под курсором темнеет — на долю `--hover--checkbox-tint`, общую с кнопкой, и тем же
цветом `--hover--checkbox-tint-color`: тёмная тема, где кнопки светлеют, светлит и отметку. У выключенного
...
| `--hover--checkbox-tint-color` | `#000` — что подмешивается, выведено из кнопочного |
...
россыпи `*_CLASS` / `*_COMMAND` / `CHANGE_EVENT` больше нет. Модуль имён не имеет побочных
эффектов: имена можно взять, не притаскивая стили и код контрола. Блок `TEXT` — подписи
по умолчанию, их переопределяет приложение (см. «Локализация»).
...
UIKIT.MODAL.TEXT.CLOSE;         // подпись крестика (см. «Локализация»)
...
и так далее. Единственное, что модуль имён импортирует, — реестр подписей `@brandup/ui-kit/i18n`
(он сам без импортов и без побочных эффектов), поэтому за одной строкой класса по-прежнему
не приходят ни стили, ни код контрола — этим путём пакеты кита берут имена друг у друга.
...
Чем пакет отдаёт наружу, записано в его `exports`: сам пакет, `./names`, `./env`, `./text`
и `./i18n` (у кита), словари подписей (`./locale/*`), стили (`./source/*.less`, `./vars.less`,
`./build/*`) и иконки. Путь внутрь `source/*.ts` не разрешается — то, на что можно опираться,
объявлено явно.

## Локализация

Подписи контролов по умолчанию английские. Приложение объявляет свои одним вызовом при старте —
до того, как контролы построятся:

```typescript
import { setTexts } from "@brandup/ui-kit/i18n";
import kitRu from "@brandup/ui-kit/locale/ru.json";
...
Русский словарь возит каждый пакет, у которого есть подписи: `@brandup/ui-kit`, `@brandup/ui-dropdown`,
`@brandup/ui-textbox`, `@brandup/ui-richeditor` и `@brandup/ui-messageeditor`. Взять можно и часть —
`setTexts` принимает любое подмножество и сливается с тем, что объявили раньше:

```typescript
setTexts({ dropdown: { PLACEHOLDER: "Выберите город" } });
```

Свои подписи типизированы: каждый пакет объявляет свой неймспейс, поэтому опечатка в имени
неймспейса или ключа не компилируется, а редактор подсказывает и то и другое. Пояснение к каждой
подписи — там же, у объявления.

### Что чем переопределяется

| Уровень | Чем задаётся | Про что говорит |
| --- | --- | --- |
| контрол | `data-*` в разметке или опция экземпляра — там, где контрол их принимает | подпись именно этого контрола: «Выберите город» |
| приложение | `setTexts` | язык приложения целиком |
| пакет | значение по умолчанию в `names.ts` | английский текст, с которым пакет поставляется |

Подпись конкретного контрола сильнее: `data-emptytext` на элементе переживает любой `setTexts`.

### Когда вызывать

Подпись попадает в разметку в момент постройки контрола, поэтому `setTexts` вызывают до того:
в старте приложения, рядом с подключением middleware. Объявленные позже подписи получат только
те контролы, которые построятся после вызова, — у собранных ранее в разметке уже лежит текст.
Смена языка на лету означает пересборку контролов.

### Свой набор подписей

Свой контрол объявляет неймспейс тем же способом, каким это делают пакеты кита, и получает
переопределение от приложения бесплатно:
...
export const GALLERY = {
    TEXT: declareTexts("gallery", { EMPTY: "No images" }),
};
```

Объект живой: читать подпись нужно там, где строится разметка. Значение, сохранённое в свою
константу на загрузке модуля, замрёт на умолчании.
