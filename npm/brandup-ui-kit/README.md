...
ничего не меняет. Отмену включает [middleware](#подключение-middleware); без неё останется только
вид. Скринридеру про закрытое поле скажет `aria-readonly` — его пишет хост. Заменой `disabled`
это не является: выключенное значение с формой не уходит, а прочитанное — уходит, ради чего
