...
export default abstract class Modal extends UIElement {
	...
	close(): void {
		// Закрыть могут и крестиком, и Esc, и подложкой — а ещё окна могло уже не стать:
		// авто-уничтожение по удалению элемента из DOM зовёт сразу destroy(), мимо close().
		// Смотрим на оба признака: на снятом окне {@link onClosing} звать нечего и незачем —
		// наследник отдаёт им результат, а отдавать его после того, как подписчики закрытия
		// отработали, значит нарушить их «ровно один раз, чем бы оно ни кончилось».
		if (this.__closed || this.__disposed) return;
		this.__closed = true;
