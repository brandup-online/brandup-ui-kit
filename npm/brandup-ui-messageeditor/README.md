...
| `blocks` | `BlockType[]` | Типы блоков сообщения: `quote`, `code`; из опций либо из атрибута `data-blocks` (по умолчанию только обычный текст) |
...
Цитата и блок кода по умолчанию выключены: их понимает не каждый канал, поэтому набор объявляет приложение — опцией `blocks` или атрибутом `data-blocks` поля-носителя (значения через пробел):

```html
<textarea data-blocks="quote code"></textarea>
```
