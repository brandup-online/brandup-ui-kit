...
У спойлера кнопки пока нет — сам инструмент работает, но в панель не выводится (см. `HIDDEN_TOOLS` в [`@brandup/ui-richeditor`](../brandup-ui-richeditor)).
...
Два вида одного значения разом на экране не держатся: панель показывается **вместо** плашки, иначе непонятно, какой из них правится. Фокус при переключении уходит из плашки — панель форматирования иначе висела бы над скрытым текстом, — а по возвращении встаёт обратно вместе с прежней кареткой. В `disabled` и `readonly` переключатель работает: смотреть значение — не значит изменять его.

```typescript
editor.toggleSource(true); // показать разметку
editor.toggleSource(); // обратно к сообщению
```
...
| `toggleSource(show?)` | Переключить режим показа; без аргумента — на противоположный. Без включённого показа выхода не делает ничего |
...
При включённом показе выхода перед плашкой встаёт переключатель `.messageeditor-modes` с кнопками `.messageeditor-mode` (выбранная — `.active`), а после неё — панель `.messageeditor-source`. Плашка и панель показываются по классу `source` на корневом элементе: он и переключает видимость.
