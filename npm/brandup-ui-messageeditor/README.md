...
| `newVariables` | `boolean` | Режим новых переменных: необъявленный ключ — не ошибка, а заявка; из опций либо из атрибута `data-new-variables` (по умолчанию выключен) |
...
- **пустой объявленный список работает** — в строгом режиме он значит «набор ещё не известен» и ничего не помечает, а здесь это рабочее начало: заводить переменные и предстоит с нуля.
...
Пустой список — не ошибка: кнопка остаётся на месте, а окно объясняет, почему выбирать нечего. Текст задаётся опцией `variablesEmpty` или атрибутом `data-variables-empty`, по умолчанию — «Переменные не заданы.» (`VARIABLES_EMPTY_TEXT`). Причину знает приложение: переменные могут появиться после выбора аудитории, а могут быть не предусмотрены вовсе. Пока список пуст, в строгом режиме ничего не проверяется и не помечается: набор ещё не известен, и чужой в нём не отличить от своего. Поля контрол тогда не касается вовсе — выставленную приложением подпись `setCustomValidity` в этом случае не стирает.
