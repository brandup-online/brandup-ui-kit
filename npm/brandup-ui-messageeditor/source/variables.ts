...
/** Свойство персонализации: подставляется приложением при отправке. */
...
/** Выбор свойства персонализации из списка; выбранное вставляется как `{ИМЯ}`. */
export default class VariablesModal extends Modal {
	 ...
	 * приложение. Список пуст, пока режим новых свойств выключен.
	 */
	constructor(
	...
	/**
	 * Запись списка. Сначала — как свойство будет выглядеть в сообщении, следом ключ: выбирают
	 * по виду, а ключ лишь уточняет, что уйдёт в текст. Без названия показывать ключ дважды
	 * незачем — вид и есть ключ.
	 *
	 * У нового свойства названия нет вовсе (в списке его нет, брать неоткуда), а место ключа
	 * занимает пометка: она объясняет и другой цвет такого свойства в тексте.
	 */
	 ...
	 * при пустом это главный выход («свойства не заданы» → настроить), при заполненном —
...
/** Оборачивает ключ (или показываемое вместо него название) в разметку свойства. */
...
/**
 * Правка ключа нового свойства.
 *
 * Объявленное выбирают из списка, а новое там взять неоткуда: его набрали здесь же — значит
 * и правят его текстом. Окно, а не правка прямо в поле: конструкция в тексте неделима (каретке
 ...
 * цельной конструкцией. Те же правила, что и у списка свойств (см. {@link isVariableKey}):
...
export class VariableKeyModal extends Modal {
	...
	constructor(text: string, apply: (text: string) => void) {
		...
		this.__field = DOM.tag("input", {
			...
			placeholder: MESSAGEEDITOR.TEXT.VARIABLE_KEY_PLACEHOLDER,
			autocomplete: "off",
			spellcheck: false,
		}) as HTMLInputElement;
...
/**
 * Разбирает список свойств из атрибута `data-variables` — когда разметку отдаёт сервер
 ...
 * как «свойства не заданы», и причину пришлось бы искать в разметке.
 *
 ...
 * что угодно, чем приложение отличает свойство), и пересобранный литерал их бы потерял —
...
export function cleanVariables(variables: MessageVariable[]): MessageVariable[] {
	...
	for (const variable of variables) {
		...
		// про его ошибку сообщаем: молча потерянное свойство выглядит как «его забыли объявить».
		if (!clean) {
			console.error("MessageEditor: свойство отброшено — негодный ключ.", variable);
			continue;
		}
...
// бы свойством на ровном месте — а `{ ИМЯ }` не совпало бы с объявленным `{ИМЯ}` никогда.
const VARIABLE_KEY = /^[\p{L}\p{N}_](?:[^\n]*[\p{L}\p{N}_])?$/u;

/**
 * Ключ, приведённый к сравнимому виду: регистр в ключах не важен. Ключ набирают руками, глядя
 * на экран, и `{скидка}` — то же свойство, что `{СКИДКА}`; считать их разными значило бы
...
/**
 * Годится ли написанное ключом свойства — ровно как есть, без чистки: буква, цифра или
 ...
 * новым свойством только тогда, когда завести его и правда можно (см. isNewVariable в
...
// Свойство без годного ключа бессмысленно, поэтому такие элементы отбрасываются.
