// Подсветка доменной разметки прямо в тексте: спинтакс `[раз|два]` и свойства `{ИМЯ}`.
...
/**
 * Объявленные свойства: ключ → название (`null` — названия нет, показывается ключ).
 *
...
export interface HighlightOptions {
	 ...
	 */
	enable?: boolean;
	/** Объявленные свойства: чего в них нет, то в тексте чужое. */
	variables?: VariableNames;
	/**
	 * Режим новых свойств: необъявленный ключ — не ошибка, а заявка на свойство, которое
	 * заведёт хост. Помечается своим классом ({@link MESSAGEEDITOR.CLASS.MARKUP.NEW}), а не как чужое.
	 */
...
// Свойство — только то, что и правда может им быть: буква, цифра или подчёркивание по краям
...
// хотя завести такое свойство нельзя.
const VARIABLE_PATTERN = /\{[\p{L}\p{N}_](?:[^{}\n]*[\p{L}\p{N}_])?\}/u;

// Выключенная персонализация не подсвечивает свойства вовсе: подсвеченная конструкция неделима,
// и без своего окна её нельзя было бы ни поправить, ни разобрать по частям.
// Флаг u — ради `\p{L}` и `\p{N}` в ключе свойства; спинтаксу он безразличен, но выражение общее.
...
export function highlight(root: HTMLElement, options: HighlightOptions = {}): boolean {
	...
	// и снимает работу с обычного набора, где ни спинтакса, ни свойств нет вовсе.
 ...
 * вставленным свойством становится нечем — а вставляют его как раз в пустое поле.
 *
...
// Из настроек подсветки длине нужна одна: объявленные свойства на счёт не влияют — считается
...
export interface LengthOptions extends Pick<HighlightOptions, "enable"> {
	/**
	 * Сколько символов считать вместо свойства: подставленное значение длиннее ключа,
	 * и точной длины у сообщения со свойствами нет — только оценка.
 ...
 * (в отправку уйдёт один из них, и лимит обязан выдержать любой), свойство — условной длиной
 * подставляемого значения. Вложенных конструкций нет (см. {@link writtenVariables}): свойство
...
/**
 * Ключ свойства по тексту найденной конструкции; null — это спинтакс, а не свойство.
 *
...
/**
 * Написанные в тексте свойства: что стоит в скобках и где сама конструкция. Спинтакс
 * не выдаётся — свойств внутри него нет: он матчится целиком, и скобки внутри него часть
 ...
 * Он же ответ на вопрос «объявлено ли такое свойство» — один источник на обе задачи.
 */
...
/**
 * Объявлено ли свойство с таким ключом.
 *
 * Список закрытый: чего в нём нет, то в тексте чужое — и пустой список не исключение.
 * Подставить такое свойство нечем, а выглядит оно ровно как рабочее, и молчать о нём значило бы
 * обещать подстановку, которой не будет. Приложение, которому набор ещё не известен (свойства
 * появляются после выбора аудитории), персонализацию до тех пор не включает: без неё `{ИМЯ}` —
 * обычный текст.
 *
 * Открытым список делает режим новых свойств: там необъявленный ключ — не ошибка, а заявка,
 * и цена ошибки — другой цвет вместо остановленной отправки. Начинают в этом режиме как раз
 * с пустого списка: свойства заводятся по мере того, как их набирают.
 */
...
/**
 * Новое ли свойство: необъявленное — и такое, каким его и правда можно завести.
 *
...
/**
 * Ключи свойств из текста, которых нет среди объявленных, — в порядке появления, без повторов.
 * Пустой список объявленных делает чужим любое свойство (см. {@link isUnknown}). В режиме новых
...
export function unknownVariables(root: HTMLElement, names?: VariableNames, newVariables?: boolean): string[] {
	...
	const found: string[] = [];
	// Повторы снимаем по сравнимому виду: `{Скидка}` и `{СКИДКА}` — одно свойство, и звать
 ...
 *
 * И в поле, и в списке свойство показывается названием — набирают его с экрана, тем, что
 * видно. Само по себе название подставить нечем: в сообщение уходит ключ, и получателю такое
 * свойство ушло бы скобками наружу. Поэтому написанное название приводим к ключу сразу,
 ...
 *
 * Ключ нового свойства приводим к верхнему регистру: объявлять его нечему, но заводить будут
 ...
 * одно свойство превращалось бы в другое. Лишние пробелы в написанном названии не важны:
...
export function mapVariableNames(root: HTMLElement, names?: VariableNames, newVariables?: boolean): boolean {
	...
	// на каждый ввод, а скобки в обычном наборе не встречаются вовсе. В режиме новых свойств
	// пустой список работе не мешает: регистр нового свойства правится и без объявленных.
 ...
 *
 * Три случая по убыванию точности: написан ключ объявленного свойства (пусть и в другом
 * регистре), написано его название, написан ключ нового свойства не в верхнем регистре.
 *
 * Ключ важнее названия — сверяем его первым: иначе написанное, совпавшее с ключом одного
 * свойства и с названием другого, превращалось бы в другое. Новое — последним: объявленное
 * название важнее догадки о новом свойстве.
 */
...
function keysByName(names: VariableNames): ReadonlyMap<string, string> {
	...
	// Одно название у двух свойств — дело хоста: на экране они неразличимы, и выбирать
...
function unwrap(root: HTMLElement) {
	root.querySelectorAll<HTMLElement>(MARKUP_SELECTOR).forEach((span) => {
		// Разворачиваем в один текстовый узел, а не в детей: у свойства с названием внутри
 ...
 *
 * У свойства с названием ключ прячется, а на экран выводится оформлением подпись из атрибута
...
function buildMarkup(text: string, options: HighlightOptions = {}): HTMLElement {
	...
	// Ключа нет среди объявленных. В строгом режиме подставить такое свойство будет нечем,
	// и получателю оно уйдёт скобками наружу; выглядит оно при этом ровно как рабочее, поэтому
	// помечаем — опечатка в ключе иначе замечается уже по отправленному сообщению. В режиме
	// новых свойств это не ошибка, а ещё не заведённое свойство: пометка другая — своим
	// цветом и своей подсказкой, — но она нужна не меньше, ведь заводить его кому-то придётся.
	...
	// ключ спрятан, а знать его иногда нужно — например когда у двух свойств одно название
...
/**
 * Свойство: скобки — своими обёртками, между ними ключ.
 *
