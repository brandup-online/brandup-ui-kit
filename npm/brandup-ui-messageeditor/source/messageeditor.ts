...
// Читать подпись отсюда можно только там, где строится разметка: объект живой, но значение,
// сохранённое в свою константу, замрёт на умолчании (см. `@brandup/ui-kit/i18n`).
const { TEXT } = MESSAGEEDITOR;
 ...
 *
 * Режимы собираются здесь, а не в константе модуля: подписи берутся из реестра при постройке
 * разметки, а константа замкнула бы их на загрузке пакета — до того, как приложение объявит свои
 * (см. `@brandup/ui-kit/i18n`).
 */
function buildModes(): { elem: HTMLElement; buttons: HTMLButtonElement[] } {
	const modes = [
		{ mode: "text", label: TEXT.MODE_TEXT, title: TEXT.MODE_TEXT_TITLE },
		{ mode: "source", label: TEXT.MODE_SOURCE, title: TEXT.MODE_SOURCE_TITLE },
	];
...
export interface MessageEditorOptions {
	/**
	 * Свойства персонализации для кнопки в панели; пусто — список в окне будет пустым.
	 *
	...
	/**
	 * Включает персонализацию: кнопку в панели, подсветку свойств и правку их окном.
	 ...
	 * объявленный там же список свойств — объявили, значит нужна.
	 */
	personalization?: boolean;
	/**
	 * Режим новых свойств: ключ, которого нет в объявленном списке, — не ошибка, а заявка
	 * на свойство, которое заведёт приложение. Отправку формы такое свойство не
	 * останавливает, в тексте помечается своим цветом (`span.variable.new`), а в окне
	 * персонализации становится записью с пометкой `VARIABLE_NEW` — чтобы его можно было вставить ещё раз.
	 ...
	 *
	 * Причину пустого списка знает приложение: свойства могут появиться после выбора аудитории,
	 * а могут быть не предусмотрены вовсе — и подсказка в этих случаях нужна разная.
	 */
	variablesEmpty?: string | null;
	/**
	 * Настройка полей персонализации: ссылка последней строкой окна свойств. Строка — адрес
	 * перехода (обычная `<a href>`), функция — действие хоста: SPA-переход или своё окно.
	 * Окно свойств при нажатии закрывается молча, без возврата каретки в поле, — фокус
	...
	/**
	 * Подпись ссылки на настройку полей; по умолчанию — подпись `VARIABLES_SETUP` из реестра
	 * (см. `@brandup/ui-kit/i18n`).
	...
	source?: boolean;
	/**
	 * Сколько символов отводится свойству при подсчёте {@link MessageEditor.messageLength};
...
export default class MessageEditor extends EditorInputControl<RichEditor, ChangeEventData, MessageEditorEvents> {
	...
	private __names: VariableNames; // названия свойств по ключу — для подсветки
	...
	constructor(valueElem: HTMLInputElement | HTMLTextAreaElement, options: MessageEditorOptions = {}) {
		...
		const emojiElem = disabled
			...
			: DOM.tag(
					"button",
					{ type: "button", class: MESSAGEEDITOR.CLASS.ELEMENT.EMOJI, title: TEXT.EMOJI },
					emojiIcon
				);
		...
		// чужое свойство от известного. В тексте показываем название, если оно задано.
		...
		// Негодная длина свойства — умолчание, а не ноль: Number("") и Number(" ") дают 0,
		...
		const editor = new RichEditor(inputElem, {
			...
			// доменные кнопки: редактор про рандомизацию и свойства не знает, только рисует их
		...
		// названия свойств ключами (см. __mapNames), и в форму значение обязано уйти с ключами.
		...
		// с чужим свойством отдаёт сервер, и до первой правки поля подписи невалидности не было
		// бы вовсе — форма ушла бы со свойством, которое нечем подставить (нативная проверка
	...
	private __initLogic() {
		...
		this.__editor.onChange((data) => {
			// Подсветка идёт до записи значения: она подменяет написанные названия свойств
	...
	/**
	 * Есть ли по чему отличать объявленное свойство от чужого. Отличать не по чему только тогда,
	 * когда персонализация выключена: `{ИМЯ}` там обычный текст, и свойств в поле нет вовсе.
	 *
	 * Пустой список этому не мешает: он значит, что не объявлено ничего, — и чужим становится любое
	 * свойство (см. isUnknown в ./highlight). Приложение, которому набор ещё не известен, включает
	 * персонализацию тогда же, когда узнаёт набор. В режиме новых свойств пустой список — рабочее
	 * начало: свойства и заводятся по мере того, как их набирают.
	 */
	...
	/**
	 * Ключи свойств из текста, которых нет в объявленном списке, — в порядке появления.
	 *
	 * В строгом режиме приложению это нужно, чтобы объяснить, что не так: подсветка показывает
	 * место, а сообщение рядом с полем — что делать. В режиме новых свойств это список того,
	...
	/**
	 * Неизвестное свойство — ошибка значения, а не оформления: подставить его нечем, и получателю
	 * оно уйдёт скобками наружу. Объявляем полю-носителю через setCustomValidity, как textbox
	 * объявляет свой лимит длины: дальше решает браузер — он же блокирует отправку формы.
	 *
	 * В режиме новых свойств проверки нет вовсе: необъявленный ключ там ожидаем, и подпись
	...
	protected override __refreshValidity(): void {
		...
		// Подстановка функцией, а не строкой: ключ может содержать `$`, а в строке замены
		// `$&` и `$'` — это узоры, и ключ съел бы сам себя.
		const keys = unknown.map(buildVariable).join(", ");
		this.__valueElem.setCustomValidity(unknown.length ? TEXT.UNKNOWN_ERROR.replace("{keys}", () => keys) : "");
	}

	/**
	 * Длина сообщения с поправкой на конструкции: спинтакс считается самым длинным вариантом
	 * (в отправку уйдёт один из них, и лимит обязан выдержать любой), свойство — условной
	...
	/**
	 * Подсветка спинтакса и свойств. Сама подсветка текст не меняет, поэтому каретка
	 ...
	 *
	 * Возвращает true, если текст поля всё же изменился — написанное название свойства стало
	...
	/**
	 * Приводит написанное название свойства к его ключу (см. {@link mapVariableNames}).
	 ...
	 * набранное с клавиатуры свойство не получалось бы стереть вовсе.
	 *
	...
	private __escapeMarkup() {
		...
		// Первый текст ищем в глубину: у свойства с названием ключ лежит в своей обёртке,
	...
	private __editMarkup(span: HTMLElement) {
		// без персонализации свойства и не подсвечиваются, но проверка дешевле, чем догадка
	...
	/**
	 * Каким окном правят конструкцию: спинтакс — рандомизацией, объявленное свойство — списком.
	 *
	 * Необъявленное в режиме новых свойств — правкой ключа: в списке его нет и быть не может,
	 * его набрали здесь же, и опечатку в нём исправляют текстом, а не выбором. В строгом режиме
	 * необъявленное — ошибка, и список там ровно то, что нужно: исправить её можно только
	 * объявленным свойством.
	 */
	 ...
	 *
	 * Новые свойства берутся из текста на каждое открытие: набранное только что должно найтись
	...
	/**
	 * Ссылка на настройку полей для окна свойств — поведение нажатия собирается здесь:
	...
	private __switch(show: boolean, focus: boolean): void {
		if (show) {
			...
			// наружу, и проверка свойств обязана пройти, как при любом таком чтении.
			this.__syncValue();
