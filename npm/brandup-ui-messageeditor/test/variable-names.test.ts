...
// Свойство набирают и вставляют с экрана — тем, что видно, а видно название. Само по себе
// оно подставить нечем: в сообщение уходит ключ, и получателю такое свойство ушло бы скобками
...
describe("variables written by name", () => {
	...
	// Ключ важнее названия: `{Имя клиента}` объявлено и ключом другого свойства, и названием
	// первого. Подмени его — набранное свойство превратилось бы в другое.
	...
	// Спинтакс матчится целиком, и свойств внутри него не ищет ни подсветка, ни проверка:
	...
	// Ключ набирают руками, глядя на экран, и `{имя}` — то же свойство, что `{ИМЯ}`. Считать
	...
	// Отправку такое поле останавливать не должно: свойство объявлено, набрано оно просто иначе.
	...
	// одно название у двух свойств — дело хоста: на экране они неразличимы, берём первое
