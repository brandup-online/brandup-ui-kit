...
не помещается и сверху места больше. Считается от окна и пересчитывается на прокрутке и изменении
размера, поэтому сторона меняется на ходу, когда кнопка уезжает к краю. Ниже `1030px` списка нет
