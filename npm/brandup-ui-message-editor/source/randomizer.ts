...
class TextRandomizer extends UIElement {
	...
	private _initEditables() {
		...
		this._elements.ownVariant.addEventListener("blur", async () => await this._refreshCandidates());
	...
	private async _refreshCandidates() {
		...
		const response: AjaxResponse<LookupModel> = await this._randomizerQueue.enque({
			method: "POST",
			url: this.api,
			headers: headers,
			query: { words: text }
